...
func get_single_best_pool{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    let (res_amount: Uint256, res_router_address, res_type) = find_best_router(
        _amount_in, _pair, Uint256(0, 0), 0, 0, 0
    );
...
@external
func payout_incentive{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(_amount) {
    // Admin.only_solvers()
    return ();
}
...
func find_best_router{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    let (res_amount, res_router_address, res_type) = find_best_router(
        _amount_in, _pair, new_best_amount[0], new_router[0], new_type[0], _counter + 1
    );
    return (res_amount, res_router_address, res_type);
}
...
func determine_best_uni_amount{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    alloc_locals;
    let (reserve1, reserve2, fee) = IUni_router.get_pool_stats(
        _router.router_address, _pair.in_token, _pair.out_token
    );
