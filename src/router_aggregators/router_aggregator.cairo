...
func get_single_best_pool{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
...
) -> (amount_out: Uint256, router_address: felt, router_type: felt) {
    ...
    let (res_amount: Uint256, res_router_address, res_type) = find_best_router(
        amount_in,
        _token_in,
        _token_out,
...
func get_liquidity_weight{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    _amount_in: Uint256, _token1: felt, _token2: felt, _router_address: felt, _router_type: felt
) -> (weight: felt) {
    alloc_locals;
    ...
    // If Uni type Dex
    if (_router_type == Uni) {
        let (reserve1: Uint256, reserve2: Uint256) = IUni_router.get_reserves(
            _router_address, _token1, _token2
        );
        ...
        let (amount_out: Uint256) = IUni_router.get_amount_out(
            _router_address, _amount_in, _token1, _token2
        );
...
func get_weight{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    // If Uni type Dex
    if (_router_type == Uni) {
        let (local amount_out: Uint256) = IUni_router.get_amount_out(
            _router_address, amount_in, _token1, _token2
        );
...
func find_best_router{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    if (router.type == Uni) {
        let (amount_out: Uint256) = IUni_router.get_amount_out(
            router.address, _amount_in, _token_in, _token_out
        );
    ...
    let (res_amount, res_router_address, res_type) = find_best_router(
        _amount_in, _token_in, _token_out, best_amount, best_router, best_type, _counter + 1
    );
    return (res_amount, res_router_address, res_type);
}

func calc_uni_amount_out{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    _amount_in: Uint256, _reserve1: Uint256, _reserve2: Uint256, _fee
) -> (amount_out: Uint256) {
