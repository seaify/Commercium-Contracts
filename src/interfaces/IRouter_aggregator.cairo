...
namespace IRouter_aggregator {
    func get_single_best_router(in_amount: Uint256, token_in: felt, token_out: felt) -> (
    ...
    func get_all_routers_and_amounts_in(_amount_out: Uint256, _token_in: felt, _token_out: felt) -> (
        amounts_in_len: felt, amounts_in: Uint256*, routers_len: felt, routers: Router*
    ) {
    }
    ...
    func get_liquidity_weight(
        _amount_in: Uint256, _src: felt, _dst: felt, _router_address: felt, _router_type: felt
    ) -> (weight: felt) {
    }
