...
func get_results{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    let (final_routers_len: felt) = kick_low_liquidity(
        sum.low, routers_len, final_routers, routers, final_liquidity, liquidity, routers_len
    );
...
func sum_liquidity{range_check_ptr}(_liquidity_len: felt, _liquidity: Liquidity*) -> (
    sum: Uint256
) {
    if (_liquidity_len == 0) {
        return (Uint256(0, 0),);
...
func kick_low_liquidity{range_check_ptr}(
    ...
    if (is_below_threshold == TRUE) {
        let (res_router_len) = kick_low_liquidity(
            _sum,
            _routers_len - 1,
            _final_routers,
            _routers + 2,
...
func set_amounts{range_check_ptr}(
    _sum: felt, _routers_len: felt, _liquidity: Liquidity*, _amounts: felt*
) {
    alloc_locals;
