...
func set_edges{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    _amount_in : Uint256, _tokens_len:felt,_tokens:felt*,_src_len:felt,_src:Source*,_dst_len:felt,_dst:felt*,_weight_len:felt,_weight:felt*,_pool_len:felt,_pool:felt*,_dst_counter:felt,_src_counter:felt,_total_counter: felt)->():
...
func determine_distances{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    ...
    _weight: felt*, 
    _dst_stop:felt, 
    _counter:felt) -> (_distances_len: felt,_distances: felt*,_queue_len: felt,_queue: felt*,_is_in_queue_len: felt,_is_in_queue: felt*):
    
    alloc_locals

    if _counter == _dst_stop:
    ...
    let (is_new_distance_better) = is_le_felt(_distances[_dst[0]],_distances[0] + _weight[0])

    if is_new_distance_better == 0:
    
        #destination vertex weight = origin vertex + edge weight
        Array.update(_distances_len,new_distances,_distances_len,_distances,_dst[0],_distances[0] + _weight[0],0)

        if _is_in_queue[_dst[0]] == 0 :
            #Add new vertex with better weight to queue
            Array.push(_queue_len+1,new_queue,_queue_len,_queue,_dst[0],0)
        ...
        assert new_distances = _distances 
        assert new_queue_len = _queue_len
        assert new_queue = _queue 
        assert new_is_in_queue = _is_in_queue
        tempvar syscall_ptr = syscall_ptr
        tempvar pedersen_ptr = pedersen_ptr
        tempvar range_check_ptr = range_check_ptr
    end

    let (res_distance_len,res_distance,res_queue_len,res_queue,res_is_in_queue_len,res_is_in_queue) = determine_distances(_distances_len, new_distances, new_queue_len, new_queue, _is_in_queue_len, new_is_in_queue, _dst_len, _dst+1, _weight_len, _weight+1, _dst_stop, _counter+1)
...
func init_arrays{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
     ...
     assert _is_in_queue[0] = 1 # In_token will start in queue
