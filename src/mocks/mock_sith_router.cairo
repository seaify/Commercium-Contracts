...
func getAmountsOut{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    let (reserve_1: Uint256, reserve_2: Uint256) = get_reserves(routes[0].from_address, routes[0].to_address);
...
@external
func swapExactTokensForTokensSimple{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        amount_in: Uint256,
    ...
    alloc_locals;
    let (amount_out: Uint256) = get_amount_out(amount_in, token_from, token_to);
